...
def get_parser():
    ...
    parser.add_argument('--config', type=str,
                        default='config/HumanBodySegmentation/HumanBodySegmentation_stratified_transformer.yaml',
                        help='config file')
    parser.add_argument('opts',
                        help='see config/HumanBodySegmentation/HumanBodySegmentation_stratified_transformer.yaml for all options',
...
def data_prepare():
    ...
    elif args.data_name == 'HumanBodySegmentation':
        features_list = sorted(glob.glob(os.path.join(args.data_root_val, "*_features.npy")))
        data_list = [os.path.basename(x).replace("_features.npy", "") for x in features_list]
    else:
...
def data_load(data_name, test_transform):
    if args.data_name == 's3dis':
        ...
        feature_path = os.path.join(args.data_root_val, data_name + '_features.npy')
        label_path = os.path.join(args.data_root_val, data_name + '_labels.npy')
        coord = np.load(feature_path).astype(np.float32)
    ...
    if test_transform:
        coord, feat = test_transform(coord, feat)
    ...
    for idx, item in enumerate(data_list):
        ...
        if os.path.isfile(pred_save_path) and os.path.isfile(label_save_path):
            ...
            for aug_id in range(len(test_transform_set)):
                ...
                for i_part in range(idx_size):
                    logger.info('{}/{}: {}/{}/{}, {}'.format(idx + 1, len(data_list), i_part + 1, idx_size,
                                                             idx_data[0].shape[0], item))
                    idx_part = idx_data[i_part]
                ...
                for i_b in range(batch_num):
                    s_i, e_i = i_b * args.batch_size_test, min((i_b + 1) * args.batch_size_test, len(idx_list))
                    idx_part_b = np.concatenate(idx_list[s_i:e_i])
                    coord_part_b = torch.FloatTensor(np.concatenate(coord_list[s_i:e_i])).cuda(non_blocking=True)
                    feat_part_b = torch.FloatTensor(np.concatenate(feat_list[s_i:e_i])).cuda(non_blocking=True)
                    offset_part_b = torch.IntTensor(np.cumsum(offset_list[s_i:e_i])).cuda(non_blocking=True)
                    with torch.no_grad():
                        offset_ = offset_part_b.clone()
                        ...
                        neighbor_idx = \
                        tp.ball_query(radius, args.max_num_neighbors, coord_part_b, coord_part_b, mode="partial_dense",
                                      batch_x=batch, batch_y=batch)[0]
                        neighbor_idx = neighbor_idx.cuda(non_blocking=True)

                        if args.concat_xyz:
                            feat_part_b = torch.cat([feat_part_b, coord_part_b], 1)

                        pred_part = model(feat_part_b, coord_part_b, offset_part_b, batch, neighbor_idx)
                        pred_part = F.softmax(pred_part, -1)
                    torch.cuda.empty_cache()
                    pred[idx_part_b, :] += pred_part
                    logger.info('Test: {}/{}, {}/{}, {}/{}, {}/{}'.format(aug_id + 1, len(test_transform_set), idx + 1,
                                                                          len(data_list), e_i, len(idx_list),
                                                                          args.voxel_max, idx_part_b.shape[0]))
