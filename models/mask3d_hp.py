...
class Mask3DHumanParts(nn.Module):
    def __init__(
        ...
        self.num_queries = self.num_human_queries * (
    ...
    def forward(
        ...
        if self.query_init == "clip_init":
            ...
            query_pos = self.pos_enc(
                ...
                input_range=[
                    scatter_min(
                        coordinates.F, coordinates.C[:, 0].long(), dim=0
                    )[0],
                    scatter_max(
                        coordinates.F, coordinates.C[:, 0].long(), dim=0
                    )[0],
            ...
            if self.non_parametric_queries:
                ...
                sampled_coords = torch.stack(
                    [
                        coordinates.decomposed_features[i][
                            fps_idx[i].long(), :
                        ]
                ...
                queries = query_pos_feat[:, : self.mask_dim, :].permute(
                    (0, 2, 1)
                )
        ...
        for decoder_counter in range(self.num_decoders):
            ...
            for i, hlevel in enumerate(self.hlevels):
                ...
                if self.restricted_cross_attention:
                    ...
                    human_mask = human_mask.repeat(
                        1, 1, self.num_parts_per_human_queries
                    )
    ...
    def mask_module(
        ...
        outputs_human_class = self.human_class_embed_head(
            query_feat[:, : self.num_human_queries, :]
        )
        outputs_part_class = self.part_class_embed_head(
            query_feat[:, self.num_human_queries :, :]
        )
    ...
    @torch.jit.unused
    def _set_aux_loss(
        self, outputs_human_class, outputs_part_class, outputs_seg_masks
    ):
        ...
        return [
            ...
            for a, b, c in zip(
                outputs_human_class[:-1],
                outputs_part_class[:-1],
                outputs_seg_masks[:-1],
            )
        ]
